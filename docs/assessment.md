...
- **Nothing to lift from their charting:** interpreters emit typed text (`%table` TSV, `%html`, `%img`, Angular directives); all rendering lives in the Angular webapp; Helium is effectively dead. Same lesson as BeakerX — rendering welded to a frontend doesn't outlive it. One caveat: the bridge maps common mimes (HTML, images) but likely drops `application/vnd.vegalite+json` — Zeppelin-facing examples should prefer HTML/SVG. *(Inferred; verify with the integration test.)*
...
- **Vega-Lite through the Zeppelin bridge** (§8) and **matrix-xchart true headlessness** (§6.1) — both inferred-likely, both one test each.
